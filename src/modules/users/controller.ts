...
export const updateProfile = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Esto debe hacerse DESPUÉS de actualizar Firestore para tener los valores finales
    // Pero lo hacemos antes para que si falla, no se actualice Firestore
    ...
    // Mantener compatibilidad con estructura anterior (user) y nueva (data)
    return res.status(200).json({
      success: true,
      message: 'Perfil actualizado correctamente',
      user: profileData,
      data: profileData, // También incluir como 'data' para mayor claridad
    });
  } catch (error) {
